...
class Magsy_Walker_Nav_Menu extends Walker_Nav_Menu {
		...
		if ( $depth == 0 && $this->mega && ( $item->object == 'category' || $item->object == 'post_tag' ) && in_array( 'menu-item-mega', $classes ) ) {
			...
			if ( $term_posts->have_posts() ) {
				...
				while ( $term_posts->have_posts() ) : $term_posts->the_post();
					...
					magsy_entry_header( array( 'container' => 'div', 'tag' => 'h4', 'date' => true ) );
	...
	public static function fallback( $args ) {
		...
		$fb_output .= '>';
		$fb_output .= '<li class="menu-item"><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '">' . esc_html__( 'Add a menu', 'magsy' ) . '</a></li>';
		$fb_output .= '</ul>';
